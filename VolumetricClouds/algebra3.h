...
//	- Added mat4*vec3
//  - Added mat3 and mat4 determinant
//  - Added vector floor and ceil
...
public:
	...
	friend vec2 floor(const vec2& v);						// term by term floor
	friend vec2 ceil(const vec2& v);						// term by term ceil
...
public:
	...
	friend vec3 floor(const vec3& v);						// term by term floor
	friend vec3 ceil(const vec3& v);						// term by term ceil
...
public:
	...
	friend vec4 floor(const vec4& v);						// term by term floor
	friend vec4 ceil(const vec4& v);						// term by term ceil
...
{ 
	return vec2(floor(v.n[VX]), floor(v.n[VY]));
}

inline vec2 ceil(const vec2& v)
{ 
	return vec2(ceil(v.n[VX]), ceil(v.n[VY]));
}
...
{ 
	return vec3(floor(v.n[VX]), floor(v.n[VY]), floor(v.n[VZ]));
}

inline vec3 ceil(const vec3& v)
{ 
	return vec3(ceil(v.n[VX]), ceil(v.n[VY]), ceil(v.n[VZ]));
}
...
{ 
	return vec4(floor(v.n[VX]), floor(v.n[VY]), floor(v.n[VZ]), floor(v.n[VW]));
}

inline vec4 ceil(const vec4& v)
{ 
	return vec4(ceil(v.n[VX]), ceil(v.n[VY]), ceil(v.n[VZ]), ceil(v.n[VW]));
}
